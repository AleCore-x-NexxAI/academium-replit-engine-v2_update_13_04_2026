...
### API Design
The API provides RESTful endpoints for managing scenarios, simulations, and user authentication. The turn processing flow involves client input, backend agent service invocation, agent workflow execution, and persistent storage of the turn, with a response to the client. The professor analytics suite includes `GET /api/professor/scenarios/:id/cohort-analytics` which aggregates class-level data: MCQ decision distributions per step, stuck nodes (NUDGE rate per decision point), reasoning style profiles (financial/people/risk/balanced based on competencyScores), and class-level competency strengths. The ScenarioAnalytics page has "Estudiantes" and "Vista de Clase" tabs.
