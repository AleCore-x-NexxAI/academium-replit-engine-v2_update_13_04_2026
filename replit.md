...
3. **Director** (`director.ts`): Orchestrates the full pipeline. Runs classifier → signal extraction → intent interpretation → parallel (Evaluator + DomainExpert) → Narrator → Causal Explainer → Assembly. Writes DecisionEvidenceLog to SimulationState. Includes graceful degradation for agent failures, decision acknowledgment (MCQ: "Opción seleccionada", FR with PRESENT+ intent: "Dirección tomada"), and 4 distinct assembly paths (PASS Intermediate, PASS Final, NUDGE, BLOCK).
4. **Evaluator + DomainExpert** run in parallel, followed by **Narrator**, then **Causal Explainer**.
5. **Domain Expert** (`domainExpert.ts`): 3-tier magnitude model (Tier 1 Ligero ±2-5, Tier 2 Moderado ±6-10, Tier 3 Significativo ±11-16 pre-authored only with auto-downgrade). Surface band → Tier 1 only. Max 3 displayed KPIs with selection priority (tier > first-appearance > signal-quality > delta). Anti-pattern detection: uniform direction flip, zero movement correction, Tier 3 auto-downgrade. KPI accumulation state per indicator: trajectory, consecutive turns, recovery attenuation (positive after negative = always Tier 1).
6. **Narrator** (`narrator.ts`): 4 required narrative elements (Observable Outcome, Stakeholder Reaction, New Information, Forward Implication). RDS-scaled richness (Surface 80-100 words, Engaged 100-130, Integrated 130-160). Compounding by position (FIRST/INTERMEDIATE/FINAL). Prohibited language scan post-generation. Tradeoff configuration states A/B/C.
7. **Causal Explainer** (`causalExplainer.ts`): NEW. 3-part structure per displayed KPI: Decision Reference (third-person, non-evaluative) + Causal Mechanism (organizational pathway) + Directional Connection (why this direction). RDS-scaled depth (Surface 2 sentences, Engaged 2-3, Integrated 3-4). Signal anchoring at STRONG level. Hint Test enforced (no prescriptive actions).

Internal agents (Director, Evaluator, SignalExtractor, InputClassifier, CausalExplainer) are hardcoded to `gpt-4o-mini`; student-facing agents (DomainExpert, Narrator) use the scenario's configured model. An on-demand explanation endpoint (`POST /api/simulations/:sessionId/explain`) lazily generates detailed causal chains for indicator changes — DomainExpert only returns `shortReason` during turn processing to keep latency low. The CausalExplainer generates structured 3-part explanations for the "¿Por qué cambió esto?" affordance on every displayed KPI movement. The architecture follows a "Stateful World, Stateless Agents" pattern, storing simulation state in PostgreSQL while agents reason without maintaining internal state.

### Data Architecture
The database schema includes tables for `users`, `scenarios`, `simulation_sessions`, `turns`, `turn_events`, and `sessions`. The `scenarios` table includes a `courseConcepts` column (`text[]`, nullable) for tagging scenarios with 3–8 course concept tags that enable concept-level analytics. The `turn_events` table provides comprehensive logging of every interaction during turn processing: rejected inputs (with verbatim student text and validator reasoning), accepted inputs, individual agent calls (inputClassifier, signalExtractor, director, evaluator, domainExpert, narrator with their outputs), turn completion summaries, and errors. Key data structures like `SimulationState` (JSONB for KPIs, history, scores, decisionEvidenceLogs, nudgeCounters, integrityFlags, indicatorAccumulation), `KPIs`, `Rubric` (JSONB for scoring criteria), and `HistoryEntry` (role-based messages) manage simulation data. `TurnResponse` now includes `displayKPIs` (max 3 per turn with direction arrows and magnitude labels), `causalExplanations` (3-part structured explanations per displayed KPI), and `decisionAcknowledgment` (optional, based on Intent signal quality). `DecisionEvidenceLog` stores per-decision signal extraction results, RDS scores/bands, and C1-C5 competency evidence. Professors can view the full event log for any session via the "Registro de Eventos" tab in the session detail dialog on ScenarioAnalytics.
